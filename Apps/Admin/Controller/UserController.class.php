...
class UserController extends BaseController {
	...
	*/
	public function index(){
		...
		// 分页处理,获取数据
		$count = $User->where ( $where )->count();//select count(*) from user  where() 
		$Page = new \Think\Page ( $count, 15 );
		$show = $Page->show ();
		
		$userArr = $User->where ( $where )->limit ( $Page->firstRow . ',' . $Page->listRows )->select();
		
		//$User->where("id = 1")->limit(0,10)->order("addrime desc")->select();
		//p($userArr);
		//dump($userArr);die;	
		...
		foreach ($userArr as $key=>$val){
			...
			//获取角色名称
			$userArr[$key]['rolename'] = $Role->getRoleName($val['role_id']);//超级管理员			
